...
- **Необязательная поддержка проекта** — переключатель на странице отправки (по умолчанию «Off», варианты 0.5% и 1%). Если включить, в подтверждении показывается точная сумма поддержки и адрес получателя — до подписи транзакции. Адрес настраивается параметром `SUPPORT_ADDRESS`.
