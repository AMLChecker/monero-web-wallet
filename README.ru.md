...
- **Никакого выбора приоритета** — все транзакции уходят с уровнем **Low**, самой дешёвой комиссией, которую
  принимает Monero; в интерфейсе нет ни переключателя, ни подписи, чтобы не гадать, сколько стоит уровень.
- **Субадреса** — создание с метками, переключение, копирование и QR-код для любого адреса.
- **Поддержка проекта** — на странице отправки предвыбран вариант **0.5%** (в один клик можно поставить «Off» или 1%). Ничего не списывается незаметно: в подтверждении всегда видно точную сумму поддержки и адрес получателя — до подписи транзакции. Адрес настраивается параметром `SUPPORT_ADDRESS`.
...
- **Страница «Support»** — кнопка рядом с Send и Receive: добровольный донат прямо на адрес разработчика, с QR-кодом
  и адресом для тех, кто хочет отправить его из другого кошелька. На донат не берётся никакой комиссии кошелька —
  уходит вся сумма, а единственная дополнительная трата это сетевая комиссия Monero, которую видно до подписи.
...
В архиве лежит проверенная версия 0.18.5.1. Когда Monero выпускает новую, `Update-Monero.bat`
(или `Start.bat --update-monero`, или `./scripts/update-monero.sh`) заменяет
`monero-wallet-rpc`, `monerod` и `monero-wallet-cli`: версия берётся из редиректа
`downloads.getmonero.org/win64`, архив проверяется по опубликованному `hashes.txt`,
заменённые файлы сохраняются в `.backup\monero-<версия>\`, а в консоль печатается, с какой
версии на какую перешли. `Update-Monero.bat 0.18.5.1` ставит конкретную версию — это же и
откат. Проверка «вышла ли новая версия» по умолчанию выключена и включается переменной
`MONERO_CHECK_UPDATES=1`; сам кошелёк по-прежнему не делает наружу никаких запросов без спроса.

## Быстрый старт (Windows)

```text
1. Распакуйте архив релиза (или просто клонируйте репозиторий — лаунчер докачает всё, чего нет)
...
**Куда положить свой кошелёк**

Положите файлы кошелька — `<имя>` и `<имя>.keys`, например `main` и `main.keys` — либо рядом с
`Start.bat`, либо в подпапку `wallets\`, и запустите кошелёк. Файл найдётся сам и появится на
первом экране кнопкой **Open `<имя>`**; пароль тот же, что в Monero CLI или GUI. Ничего не
перемещается, не переименовывается и не копируется: эта папка просто передаётся в
`monero-wallet-rpc --wallet-dir`. Если кошельки лежат в другом месте, укажите его в
`MONERO_WALLET_DIR`.
